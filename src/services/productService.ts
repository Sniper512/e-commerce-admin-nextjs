...
export const cartonService = {
	// Get all cartons
	async getAll(productId?: string): Promise<Carton[]> {
		const constraints: QueryConstraint[] = [];
		...
		const q = query(collection(db, CARTONS_COLLECTION), ...constraints);
		const snapshot = await getDocs(q);
	...
	// Create carton
	async create(
		data: Omit<Carton, "id" | "createdAt" | "updatedAt">
	): Promise<string> {
		const docRef = await addDoc(collection(db, CARTONS_COLLECTION), {
			...data,
	...
	// Update carton
	async update(id: string, data: Partial<Carton>): Promise<void> {
		const docRef = doc(db, CARTONS_COLLECTION, id);
		const updateData: any = {
			...data,
			updatedAt: Timestamp.now(),
		};
