...
export const BatchService = {
    ...
    async getExpiringBatches(daysThreshold: number = 30): Promise<Batch[]> {
        try {
            ...
            const batches = snapshot.docs.map((doc) => firestoreToBatch(doc.id, doc.data()));
    ...
    // Create batch
    async createBatch(batchData: Omit<Batch, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
        try {
            const batchesRef = collection(db, BATCHES_COLLECTION);
    ...
    async updateBatch(id: string, batchData: Partial<Batch>): Promise<void> {
        try {
            ...
            if (batchData.productId !== undefined) data.productId = batchData.productId;
            if (batchData.productName !== undefined) data.productName = batchData.productName;
            if (batchData.manufacturingDate !== undefined) {
                data.manufacturingDate = Timestamp.fromDate(batchData.manufacturingDate);
            }
            ...
            if (batchData.updatedBy !== undefined) data.updatedBy = batchData.updatedBy;
