...
export function DiscountSearchDropdown({
    ...
    // Memoize filtered discounts to prevent unnecessary recalculations
    const filteredDiscounts = useMemo(() => {
        if (!searchValue) return availableDiscounts;
    ...
    }, [searchValue, availableDiscounts]);
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            {isOpen && filteredDiscounts.length > 0 && (
                    ...
                    {filteredDiscounts.map((discount) => {
                        ...
                        return (
                            <button
                                ...
                                className={`w-full flex items-center gap-3 px-4 py-2 hover:bg-gray-100 text-left ${!isActive ? 'opacity-50' : ''
                                    }`}
                                disabled={!isActive}
                            >
            ...
            {isOpen && filteredDiscounts.length === 0 && searchValue && (
                <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg p-4 text-center text-gray-500 text-sm">
                    No discounts found matching "{searchValue}"
                </div>
            )}
        </div>
    );
