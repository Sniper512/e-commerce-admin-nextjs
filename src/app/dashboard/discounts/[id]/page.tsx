...
export default function DiscountDetailPage() {
  ...
  const handleDelete = async () => {
    if (
      !confirm(
        "Are you sure you want to delete this discount? This will remove the discount from all associated products."
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (
        ...
        const updatePromises = discount.applicableProducts.map(
          async (productId) => {
            try {
              const product = await productService.getById(productId);
              if (product) {
                const existingDiscountIds = product.pricing.discountIds || [];
                const updatedDiscountIds = existingDiscountIds.filter(
      ...
      if (
        ...
        const updatePromises = discount.applicableCategories.map(
          async (categoryId) => {
            try {
              ...
              console.error(
                `Error removing discount from category ${categoryId} products:`,
                error
              );
            }
          }
  ...
  const usagePercentage =
    discount.limitationType !== "unlimited" && discount.limitationTimes
      ? Math.round(
          ((discount.currentUsageCount || 0) / discount.limitationTimes) * 100
        )
      : 0;
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex flex-col items-end space-y-2">
              ...
              {isUpcoming && <Badge className="bg-yellow-500">Upcoming</Badge>}
              {isExpired && <Badge className="bg-red-500">Expired</Badge>}
            </div>
          </div>
        </CardHeader>
      </Card>
