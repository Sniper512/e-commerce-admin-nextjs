    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (formData.applicableTo === 'products' && formData.applicableProducts.length > 0) {
                const updatePromises = formData.applicableProducts.map(async (productId) => {
                    try {
                        ...
                        console.error(`Error updating product ${productId} with discount:`, error);
                    }
                });
            ...
            if (formData.applicableTo === 'categories' && formData.applicableCategories.length > 0) {
                const updatePromises = formData.applicableCategories.map(async (categoryId) => {
                    try {
                        ...
                        if (category && category.productIds.length > 0) {
                            ...
                            const productUpdatePromises = category.productIds.map(async (productId) => {
                                try {
                                    ...
                                    console.error(`Error updating product ${productId} in category:`, error);
                                }
                            });
                            await Promise.all(productUpdatePromises);
                        }
                    } catch (error) {
                        console.error(`Error updating category ${categoryId} products:`, error);
                    }
                });
