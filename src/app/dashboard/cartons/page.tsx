...
export default function CartonsPage() {
    ...
    const [statusFilter, setStatusFilter] = useState<'all' | 'active' | 'archived' | 'shipped'>('all');
    ...
    const filterCartons = () => {
        ...
        if (searchTerm) {
            ...
            filtered = filtered.filter(carton =>
                ...
                carton.products.some(p => p.productName.toLowerCase().includes(lowercaseSearch))
            );
        }
            ...
            <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-gray-600">Manage inventory cartons with multiple products</p>
                    </div>
                                        ...
                                        {filteredCartons.map((carton) => (
                                            <tr key={carton.id} className="hover:bg-gray-50">
                                                <td className="px-6 py-4">
                                                    <div className="font-medium text-gray-900">{carton.cartonNumber}</div>
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="text-sm">{carton.products.length} items</div>
                                                    <div className="text-xs text-gray-500">
                                                        {carton.products.slice(0, 2).map(p => p.productName).join(', ')}
                                                        {carton.products.length > 2 && '...'}
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4 font-medium">{carton.totalQuantity}</td>
                                                <td className="px-6 py-4 font-medium">{formatCurrency(carton.totalCost)}</td>
                                                <td className="px-6 py-4">{getStatusBadge(carton.status)}</td>
                                                <td className="px-6 py-4 text-sm">{carton.supplier || '-'}</td>
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <Button
                                                            variant="ghost"
                                                            size="sm"
                                                            onClick={() => handleDelete(carton.id, carton.cartonNumber)}
                                                        >
