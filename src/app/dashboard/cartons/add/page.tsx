...
import { CartonProductItem } from "@/types";
...
export default function AddCartonPage() {
      ...
      prev.map((product) => {
        if (product.tempId === tempId) {
          ...
          updatedProduct.totalCost =
            updatedProduct.quantity * updatedProduct.costPerUnit;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const invalidProducts = products.filter(
      (p) => p.quantity <= 0 || p.costPerUnit <= 0
    );
    if (invalidProducts.length > 0) {
      alert(
        "All products must have a quantity and cost per unit greater than 0"
      );
      return;
    }
    ...
    try {
      ...
      const cleanProducts: CartonProductItem[] = products.map(
        ({ tempId, ...rest }) => rest
      );
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold">Add New Carton</h1>
            <p className="text-gray-600">
              Create a new carton with multiple products
            </p>
          </div>
        </div>
      </div>
